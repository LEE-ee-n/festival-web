...
- 탈퇴 완료 안내는 상태색 박스 대신 로그인 화면 중앙의 회색 문구로 표시한다.
- 삭제된 계정의 JWT가 다른 브라우저 출처에 남아 있으면 `user_not_found`를 감지해 해당 출처의 세션을 제거하고 메인페이지로 이동한다.
- 삭제 사용자 복구 자동 테스트에서 세션 삭제 후 메인 이동, 메인에서 중복 이동 방지, 다른 인증 오류 무개입을 확인했다.
