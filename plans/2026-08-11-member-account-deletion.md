...
- Vercel Production 서버 환경변수에 서버 전용 Supabase 비밀키를 설정하고 재배포했다.
- 임시 일반 회원으로 운영 E2E 탈퇴를 완료했다. 탈퇴 후 개인 데이터 소멸, 같은 Google 계정 재가입 시 기록 미복구, 삭제 계정의 오래된 세션 정리와 메인 이동을 확인했다.
