...
8. [완료] UI 변경을 운영에 먼저 배포한다.
9. [완료] 운영 관리자 계정에 TOTP를 등록하고 실제 로그인을 확인한다.
10. [완료] verified factor와 `aal2`를 확인한 뒤 운영 DB migration을 적용한다.
11. [완료] `aal1`·`aal2` 관리자, 일반 회원과 Bot 회귀를 검증하고 문서를 완료 상태로 갱신한다.
...
- GitHub 커밋 `d1079b8`의 Vercel 배포 성공과 운영 `/admin/login` 화면을 확인했다.
- 운영 관리자 TOTP factor는 verified 1개, unverified 0개이며 사용자가 실제 로그인 성공을 확인했다.
- 운영 DB에 `enforce_admin_aal2` migration을 적용하고 migration 이력을 확인했다.
- DB 역할 시험에서 관리자 `aal1` 거부, 관리자 `aal2` 허용, 일반 회원 `aal2` 거부를 확인했다.
- Bot 전용 역할은 유지되고 관리자 접근은 거부됐으며 상태 갱신 cron은 활성·최근 `succeeded` 상태다.
