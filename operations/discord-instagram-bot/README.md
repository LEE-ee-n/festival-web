...
Discord에 Instagram 게시물 URL을 보내면 전체 이미지와 캡션을 분석하고 웹 단계형 임시등록을 만듭니다. Discord에는 JSON·WebP를 첨부하지 않고 관리자 작업함 링크만 표시합니다. 첫 이미지는 승인 전 검토용 WebP로 임시 저장하고 최종 등록·수정 후 삭제하며 정식 썸네일로 자동 승격하지 않습니다. 일반 게시글의 작성자 프로필은 공식 Instagram URL 검토값으로 함께 저장합니다.
