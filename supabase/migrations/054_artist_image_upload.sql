...
drop policy if exists "Admins can select artist images" on storage.objects;
...
create policy "Admins can select artist images"
...
declare
  ...
  v_image_url text := case
    when p_image_url is null then null
    else nullif(pg_catalog.btrim(p_image_url), '')
  end;
  v_alias text;
...
begin
  ...
  if v_instagram_url is not null
    and v_instagram_url !~ '^https://(www[.])?instagram[.]com/[^/?#]+/?([?].*)?$'
  then
    raise exception 'Instagram 공식 프로필 URL만 입력할 수 있습니다.' using errcode = '22023';
  end if;

  if v_featured_playlist_url is not null
    and v_featured_playlist_url !~ '^https://(www[.])?(youtube[.]com|music[.]youtube[.]com)/playlist[?][^#]*list=[^&#]+'
  then
    raise exception 'YouTube 또는 YouTube Music 재생목록 URL만 입력할 수 있습니다.' using errcode = '22023';
  end if;

  if v_image_url is not null
    and v_image_url !~ '^https://[^[:space:]]+[.]webp([?][^[:space:]]*)?$'
  then
    raise exception '아티스트 로고는 HTTPS WebP URL이어야 합니다.' using errcode = '22023';
  end if;
  ...
  if exists (
    ...
    where normalized_name = v_normalized_name and id <> p_artist_id
  ) then
  ...
  if exists (
    ...
    join pg_catalog.unnest(coalesce(p_aliases, '{}'::text[])) input_alias(alias_name)
      on pg_catalog.lower(pg_catalog.btrim(aa.alias_name))
    ...
    raise exception '입력한 별칭이 다른 아티스트에 등록되어 있습니다.' using errcode = '23505';
  end if;
  ...
  select coalesce(
    pg_catalog.array_agg(alias_name order by pg_catalog.lower(alias_name)),
    '{}'::text[]
  )
  into v_old_aliases
  ...
  if v_before.name is distinct from v_name
    ...
    or (p_image_url is not null and v_before.image_url is distinct from v_image_url)
  then
    update public.artists set
      name = v_name,
      normalized_name = v_normalized_name,
      instagram_url = case
        when p_instagram_url is null then instagram_url else v_instagram_url
      end,
      featured_playlist_url = case
        when p_featured_playlist_url is null then featured_playlist_url else v_featured_playlist_url
      end,
      image_url = case
        when p_image_url is null then image_url else v_image_url
      end
    where id = p_artist_id;
  end if;
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'instagram_url', case
      when p_instagram_url is null then v_before.instagram_url else v_instagram_url
    end,
    'featured_playlist_url', case
      when p_featured_playlist_url is null then v_before.featured_playlist_url else v_featured_playlist_url
    end,
    'image_url', case
      when p_image_url is null then v_before.image_url else v_image_url
    end
  );
end;
$$;

revoke all on function public.update_artist_admin(
  bigint,
  text,
  text,
  text[],
...
grant execute on function public.update_artist_admin(
  bigint,
  text,
  text,
  text[],
