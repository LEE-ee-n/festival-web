...
add column if not exists instagram_url_unavailable boolean not null default false,
add column if not exists official_url_unavailable boolean not null default false;

comment on column public.festivals.instagram_url_unavailable is
  '공식 Instagram 계정이 실제로 없음을 관리자가 확인한 상태';

comment on column public.festivals.official_url_unavailable is
  '공식 홈페이지가 실제로 없음을 관리자가 확인한 상태';

create or replace function public.clear_festival_link_unavailable_on_url()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if nullif(pg_catalog.btrim(new.instagram_url), '') is not null then
    new.instagram_url_unavailable := false;
  end if;

  if nullif(pg_catalog.btrim(new.official_url), '') is not null then
    new.official_url_unavailable := false;
  end if;
...
drop trigger if exists festivals_clear_link_unavailable_on_url
on public.festivals;

create trigger festivals_clear_link_unavailable_on_url
before insert or update of instagram_url, official_url,
  instagram_url_unavailable, official_url_unavailable
on public.festivals
for each row
execute function public.clear_festival_link_unavailable_on_url();
...
revoke all on function public.update_festival_basic_info_with_audit(bigint, jsonb)
from public, anon;

grant execute on function public.update_festival_basic_info_with_audit(bigint, jsonb)
to authenticated;
