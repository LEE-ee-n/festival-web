...
- artists.normalized_name: 아티스트 중복 판별값. NOT NULL·UNIQUE이며 영문 소문자와 숫자만 허용
  - 아티스트 관리, 수집 후보, JSON, 엑셀 등록에서 같은 정규화 함수를 사용
  - 영문 이름이면 자동 생성하고, 신규 한글 이름만으로 생성할 수 없으면 등록을 차단
...
- 축제 기본정보·라인업·티켓 관리
- JSON 축제 트랜잭션 등록
- 사진·게시글 기반 JSON 수집 후보 저장·폼 검토·승인 등록
- 후보 아티스트 `normalized_name` 기준 중복 확인
- 전체 아티스트 ID·표시 이름·고유 이름·별칭 조회 및 통합 수정
