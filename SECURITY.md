...
2026-08-11 코드, 운영 DB 구조와 외부 서비스 구성을 대조한 기준이다. Supabase 프로젝트의 기본 데이터 리전은 대한민국 서울(`ap-northeast-2`)이다.

| 흐름 | 처리 항목 | 목적·보유 위치 | 보유·삭제 기준 |
| --- | --- | --- | --- |
| Google 로그인 → Supabase Auth | 이메일, Google 제공자 식별정보, 제공되는 경우 이름·프로필 이미지, 가입·최근 로그인 일시 | 회원 식별과 세션 유지. Google 및 Supabase Auth에서 처리 | 회원탈퇴 시 Auth 사용자 삭제 |
| 브라우저 → Supabase DB | 관심 축제·아티스트, 공연 일정, 페스티벌 기록·메모·곡·외부 미디어 참조, 베타 이용권 | 개인 서비스 제공. 서울 리전의 Supabase DB에 저장 | 회원 유지 기간. 탈퇴 시 FK cascade로 활성 데이터에서 삭제 |
| 운영 DB → 수동 백업 | 공개 운영 데이터와 개인 데이터가 포함될 수 있는 DB 덤프 | 재해 복구. 운영 DB와 분리된 운영자 저장소 | 최근 4주. 복원 시 이미 탈퇴한 회원 정보 재삭제 |
| 브라우저 → Vercel·Supabase | IP, 접속 일시, 요청 경로, 브라우저·기기, 오류·인증·보안 로그 | 호스팅, API 제공, 장애·부정 접근 확인 | 공급자 요금제·설정에 따른 기간. Festibom이 별도 저장하면 최대 90일 |
| 브라우저 → GA4·Clarity | 방문·유입·기기, 페이지 이동, 클릭·스크롤 | 공개 페이지 이용 분석. 환경변수가 설정되고 이용자가 `분석 허용`을 선택한 경우에만 처리 | GA4 사용자·이벤트 데이터 최대 14개월, Clarity 재생 최대 30일 및 히트맵·표시 세션 최대 9개월 기준 |
| 이용자 → Gmail | 발신 이메일, 문의·제보 내용, 이용자가 첨부한 자료 | 문의·정보 수정·권리 요청 처리 | 처리 완료일부터 최대 1년 |

- 일반 회원 파일 업로드 기능은 없고 운영 일반 회원 소유 Storage 객체도 0건으로 확인했다.
- Google provider token은 저장하거나 Google API 호출에 사용하지 않는다.
- GA4·Clarity에는 Supabase 회원 식별자를 별도 사용자 ID로 전송하지 않는다.
- 분석 선택 전이나 `필수만 사용` 상태에는 GA4·Clarity 스크립트를 로드하지 않는다. 선택은 브라우저에만 저장되고 푸터 `분석 설정`에서 변경할 수 있다.
- 공급자 대시보드의 실제 로그·분석 보유 설정이 바뀌면 개인정보처리방침과 이 표를 함께 갱신한다.

## 보안 사고 초기 대응 체크리스트

### 공통 0~1시간

1. 발견 시각, 최초 신고자, 영향 서비스와 증상을 기록하고 원본 로그를 보존한다. 로그나 문서에 비밀키와 개인정보 원문을 복사하지 않는다.
2. 운영 변경 담당자를 한 명으로 정하고 무관한 배포·DB 작업을 중지한다.
3. 영향을 받은 계정·키·경로만 우선 차단하고, 증거 보존 전에 로그·계정·DB를 일괄 삭제하지 않는다.
4. 유출 가능 개인정보 항목, 이용자 수, 시작·종료 시각, 외부 불법 접근 여부를 확인한다.

### 사고 유형별 차단

- 개인정보 유출: 노출 API·RLS·다운로드 경로를 차단하고 관련 세션을 폐기한다. 영향 행과 접근 로그를 보존한다.
- 관리자 계정 탈취: 관리자 세션과 MFA factor를 폐기하고 Supabase·Google·GitHub·Vercel 계정 접근을 재검증한다. 관리자 비밀번호와 복구 수단을 교체한다.
- Bot·서버 키 노출: 노출된 키를 제공자에서 즉시 폐기·재발급하고 Vercel·Bot 실행 환경만 새 값으로 갱신한다. Git과 로그의 노출 범위를 다시 검사한다.
- DB 장애·오삭제: 쓰기 작업을 중지하고 현재 상태와 migration 이력을 보존한다. 운영 DB를 즉시 덮어쓰지 않고 격리된 프로젝트에서 백업 복구와 무결성을 먼저 확인한다.

### 72시간 통지·신고 판단

1. 개인정보 유출 사실을 알게 되면 정당한 사유가 없는 한 72시간 이내 이용자 통지를 준비한다.
2. 1천 명 이상, 민감정보·고유식별정보 포함, 또는 외부 불법 접근에 의한 유출이면 개인정보보호위원회 또는 KISA 신고 대상인지 즉시 확인한다.
3. 통지·신고에는 유출 항목과 규모, 시점·경위, 피해 최소화 조치, 이용자 대응·구제 절차와 담당자 연락처를 포함한다.
4. 사실관계가 모두 확정되지 않았더라도 법정 기한을 넘기지 않도록 확인된 내용으로 우선 통지하고 후속 내용을 보완한다.

### 복구와 사후 조치

1. 새 키·정책·배포를 적용한 뒤 관리자 MFA, 일반 회원 교차 계정 RLS, 탈퇴와 핵심 공개 화면을 다시 시험한다.
2. 격리 복구 데이터의 행 수, FK 관계와 Storage 파일 무결성을 확인한 뒤 별도 승인으로 운영 복구한다.
3. 원인, 영향, 조치 시각, 재발 방지 담당자와 완료 기한을 기록한다.
4. 개인정보처리방침·보유기간·외부 처리 현황이 실제 구성과 달랐다면 즉시 수정하고 변경 내용을 공지한다.

법적 판단 기준은 개인정보 보호법 제21조(파기), 제28조의8(국외 이전), 제29조(안전조치), 제30조(처리방침), 제34조(유출 통지·신고)와 개인정보보호위원회 안내를 사용한다. 실제 사고에서는 규모와 정보 유형에 따라 전문가 또는 KISA 118에 즉시 확인한다.
