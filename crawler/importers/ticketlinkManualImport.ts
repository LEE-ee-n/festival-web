...
export function importTicketlinkManualCandidates(
  input: unknown,
  discoveredAt: string,
): CrawledCandidate[] {
  if (!Array.isArray(input)) throw new Error("티켓링크 입력 JSON은 배열이어야 합니다.");
  ...
  for (const raw of input as ManualLink[]) {
    ...
    const pathMatch = url.pathname.match(TICKETLINK_PRODUCT_PATH);
    if (url.protocol !== "https:" || url.hostname !== "www.ticketlink.co.kr" || !pathMatch) continue;
    const title = raw.title.replace(/\s+/g, " ").trim();
    const rawTitle = typeof raw.raw_title === "string"
      ? raw.raw_title.replace(/\s+/g, " ").trim()
      : "";
    ...
    const festivalTitle = isTicketlinkFestivalTitle(title)
      ? title
      : isTicketlinkFestivalTitle(titleFromRaw)
        ? titleFromRaw
        : "";
    if (!festivalTitle) continue;
    const startDate = typeof raw.start_date === "string" && ISO_DATE.test(raw.start_date)
      ? raw.start_date
      : undefined;
    const endDate = typeof raw.end_date === "string" && ISO_DATE.test(raw.end_date)
      ? raw.end_date
      : startDate;
    if (collectedDate && endDate && endDate < collectedDate) continue;
    const sourceUrl = `https://www.ticketlink.co.kr/product/${pathMatch[1]}`;
    candidates.push({
      title: festivalTitle,
      source_url: sourceUrl,
      source_type: "ticketlink_manual",
