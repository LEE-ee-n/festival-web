...
const inputPath = await findLatestTicketlinkInput([crawlDirectory]);
const date = collectionDateFromFilename(basename(inputPath), new Date());
const archivedInputPath = resolve(crawlDirectory, `${date}-TICKETLINK-crwal.json`);
const outputPath = resolve(discoveryDirectory, `${date}-TICKETLINK-discovery.json`);
...
const report: CrawlerReport = {
  ...
  items: importTicketlinkManualCandidates(raw, generatedAt),
  errors: [],
};
...
process.stdout.write(`${report.items.length}개 티켓링크 후보를 정리했습니다.\n`);
